    ...
    public handleOAuthRedirect(SalesForceDomainPrefix, req, res) {
        return new Promise((resolve, reject) => {
            ...
            cloudElements.createInstance(SalesForceDomainPrefix, req.query.code).then((result: any) => { 
                ...
                //let accountData = {userId: req.query.userId, CEelementInstanceId: result.id, CEelementInstanceToken: result.token, identifier: null, vendorParameter: '', organizationId: '', email: '', vendorId: ''};
                ...
                var vendorId: String = result.configuration['sfdc.user.id.url'];
                ...
                let accessToken = {vendorId: accountData.vendorId,
                                    ...
                                    authorizationUrl: result.configuration['sfdc.revoke.url'], //revoke
                ...
                hubx2.memory.createAccount(accountData.vendorId, hubXConfiguration.accountType, '', "oAuth2", accountData, accountData.userId, 0, accountData.organizationId).then((accountResult) => {
                    ...
                    let accountIdentifier: number = Number(accountData.identifier); //accountData.userId;
                        ...
                        if(!(accountData.CEelementInstanceId==previousData.CEelementInstanceId)) {
                            hubx2.memory.updateAccessToken(accountResult.accessTokenId, accessToken, false).then((newToken) => {
                                ...
                                hubx2.memory.updateAccount(accountIdentifier, hubXConfiguration.accountType, {data: accountData}).then(() => {
                                    ...
                                    cloudElements.deleteCEInstance(previousData.CEelementInstanceId).then(() => { //delete old instance, keep new and update hubx account data
